...
class TicketAnalyzerService:
    ...
    def _generate_response(self, ticket: Ticket, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            """
            Générer une réponse IA structurée (JSON)
            Compatible avec LLMClient (string -> json.loads)
            """
            ...
            system_prompt = (
                "Tu es un expert support. "
                "Tu DOIS répondre uniquement en JSON valide, sans balises, sans texte."
            )
            ...
            if not result.get("success"):
                return {
                    ...
                    "solution_steps": [
                        ...
                        "Retour vers vous avec confirmation"
                    ]
                }

            raw_content = result.get("content", "").strip()

            # Nettoyage au cas où l’IA ajoute ```json
            ...
            try:
                ...
                logger.error(f"JSON invalide retourné par LLM: {raw_content}")
                return {
                    "response_text": "Votre demande est prise en charge par notre équipe.",
                    "solution_steps": [
                        "Analyse en cours",
